...
This register tracks operational, security, and reliability risks for the UC Self-Service application (v0.2.0).  
...
|----|------|------------|--------|--------|----------------------|-------|--------|
| R1 | Admin endpoints exposed due to misconfiguration | Low | High | Medium | Nginx Basic Auth + Flask admin allowlist; restrict `/admin` access; verify configuration during deployment | UC Self-Service Maintainer | Mitigated |
| R2 | SQLite file corruption or data loss | Low | Medium | Medium | Regular backups and tested restore procedure; consider migration to PostgreSQL for production | UC Self-Service Maintainer | Open |
...
| R5 | Single host failure (Raspberry Pi) | Low | High | Medium | Maintain system image backup; document recovery steps; consider VM or HA deployment for production | UC Self-Service Maintainer | Open |
...
| R12 | Lack of rate limiting enabling abuse or accidental overload | Low | Medium | Low | Add Nginx rate limiting if exposed to large user base | UC Self-Service Maintainer | Future |
